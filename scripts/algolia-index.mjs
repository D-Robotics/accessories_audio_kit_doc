...
const INDEX_NAME = (
  process.env.ALGOLIA_INDEX_NAME || "accessories_audio_kit_doc"
).trim();
const SITE_URL = normalizeSiteUrl(
  process.env.ALGOLIA_SITE_URL ||
    "https://developer.d-robotics.cc/accessories_audio_kit_doc/",
);
...
async function fetchHtml(url) {
  const res = await fetch(url, {
    headers: {
      "User-Agent": "accessories_audio_kit_doc-algolia-indexer/1.0",
